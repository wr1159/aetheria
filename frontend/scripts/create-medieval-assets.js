...
// Create foreground (decorative elements that appear in front of the player)
const foregroundCanvas = createCanvas(800, 600);
const foregroundCtx = foregroundCanvas.getContext('2d');

// Make background transparent
foregroundCtx.clearRect(0, 0, 800, 600);

// Draw some decorative elements that will appear in front of the player
// Example: Draw some tall grass patches
foregroundCtx.fillStyle = '#90EE90'; // Light green color
for (let i = 0; i < 20; i++) {
    ...
    // Draw grass blade
    foregroundCtx.fillRect(x, y, width, height);
    
    // Add slight curve to grass
    foregroundCtx.beginPath();
    foregroundCtx.moveTo(x, y);
    foregroundCtx.quadraticCurveTo(x + width/2, y + height/2, x, y + height);
    foregroundCtx.strokeStyle = '#90EE90';
...
for (let i = 0; i < 15; i++) {
    ...
    // Draw flower petals
    foregroundCtx.fillStyle = '#FFB6C1'; // Light pink
    for (let j = 0; j < 5; j++) {
        ...
        foregroundCtx.beginPath();
        foregroundCtx.arc(petalX, petalY, 3, 0, Math.PI * 2);
        foregroundCtx.fill();
    }
    
    // Draw flower center
    foregroundCtx.fillStyle = '#FFD700'; // Gold
    foregroundCtx.beginPath();
    foregroundCtx.arc(x, y, 2, 0, Math.PI * 2);
    foregroundCtx.fill();
}

// Save foreground image
fs.writeFileSync(`${assetsDir}/foreground.png`, foregroundCanvas.toBuffer('image/png'));
console.log('Created foreground.png');
